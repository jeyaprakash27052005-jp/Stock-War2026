...
export const LoginView: React.FC<LoginViewProps> = ({ onStudentLogin, onTeacherLogin }) => {
  ...
  const handleStudentSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const roll = studentUserId.trim().toUpperCase();
    const pass = studentPassword.trim();

    if (!roll) {
      setError('Please enter your Student User ID (e.g. 25MBA01).');
      return;
    }
    if (!pass) {
    ...
    // Validation: Student User ID and Password must be the same
    if (pass.toUpperCase() !== roll) {
      setError('Invalid password. Student User ID and Password must be the same (e.g., User ID: 25MBA01, Password: 25MBA01).');
      return;
    }
    ...
    <div className="flex-1 flex items-center justify-center p-6 bg-[radial-gradient(ellipse_900px_500px_at_50%_-10%,rgba(212,169,63,0.08),transparent)]">
      <div className="w-full max-w-md bg-[#10161D] border border-[#1F2A33] relative shadow-2xl">
            ...
            <form onSubmit={handleStudentSubmit} className="space-y-4">
              <div>
                ...
                <div className="relative">
                  <input
                    type="text"
                    id="studentUserIdInput"
                    value={studentUserId}
                    onChange={(e) => setStudentUserId(e.target.value)}
                    placeholder="e.g. 25MBA01"
                    autoComplete="username"
                    autoCapitalize="characters"
                    className="w-full bg-[#141B23] border border-[#1F2A33] text-[#F1F4F6] px-3.5 py-2.5 font-['IBM_Plex_Mono',monospace] text-sm tracking-wider outline-none focus:border-[#D4A93F] transition-colors"
                  />
                </div>
                <div className="text-[11px] text-[#6B7680] mt-1.5">
                  Enter your assigned roll number or registration ID.
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="Same as User ID (e.g. 25MBA01)"
                ...
                <div className="text-[11px] text-[#D4A93F] mt-1.5 flex items-center gap-1.5">
                  <Lock className="w-3 h-3" />
                  <span>Your password is the same as your User ID (e.g. 25MBA01).</span>
                </div>
              </div>
              ...
              >
                {loading ? 'Accessing Floor...' : 'Enter Trading Floor'}
              </button>
            </form>
          ) : (
        ...
        <div className="text-center text-[11px] text-[#6B7680] p-4 border-t border-[#1F2A33] border-dashed">
          {activeTab === 'student'
            ? 'Student balances & positions are preserved in Cloud Firestore.'
